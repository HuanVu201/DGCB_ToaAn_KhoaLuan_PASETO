...
export const SuaTieuChiDanhGiaCon = ({
  handlerClose,
  visible,
  folderId,
}: {
  ...
  const { danhSachTieuChiDanhGia: tieuchidanhgias, data: tieuchidanhgia } =
    useAppSelector((state) => state.tieuchidanhgia);
  ...
  useEffect(() => {
    dispatch(GetTieuChiDanhGia(folderId));
  }, []);

  useEffect(() => {
    if (tieuchidanhgia) {
      let loaiDiem;
      if (tieuchidanhgia.diemLiet) {
        loaiDiem = "diemLiet";
      } else if (tieuchidanhgia.diemThuong) {
        loaiDiem = "diemThuong";
      } else if (tieuchidanhgia.diemTru) {
        loaiDiem = "diemTru";
      } else {
        loaiDiem = "diemDatYeuCau";
      }
      if (tieuchidanhgia.jsonLienKet != null)
        setLstDoiLap(JSON.parse(tieuchidanhgia.jsonLienKet));
      if (tieuchidanhgia.jsonDiemLiet != null)
        setLstDiemLiet(JSON.parse(tieuchidanhgia.jsonDiemLiet));
      form.setFieldsValue({ ...tieuchidanhgia, loaiDiem});
    }
  }, [tieuchidanhgia]);
  ...
  const onOk: FormProps["onFinish"] = async () => {
    ...
    //  const res =  await TieuChiDanhGiaApi.Create({data : formData})
    ...
    const response = await dispatch(
      UpdateTieuChiDanhGiaOld({
        ...
        data: {
          ...formData,
          maDayDu: parentTieuChi?.maDayDu,
          maTieuChi: parentTieuChi?.maTieuChi,
          maMauPhieuDanhGia: parentTieuChi?.maMauPhieuDanhGia,
          diemLiet: pointLiet,
          diemTru: pointTru,
