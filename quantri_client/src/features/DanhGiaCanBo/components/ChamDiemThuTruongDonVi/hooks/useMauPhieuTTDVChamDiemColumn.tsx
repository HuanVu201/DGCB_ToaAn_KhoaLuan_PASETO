...
const updateParentScores = (data: IDanhGiaColumn[]): IDanhGiaColumn[] => {
  const updateScoreTuDG = (item: IDanhGiaColumn): number => {
    if (item.DanhSachTieuChiCon && item.DanhSachTieuChiCon.length > 0) {
      const childrenSum = item.DanhSachTieuChiCon.reduce((sum, child) => sum + updateScoreTuDG(child), 0);
  ...
  const updateScoreDG = (item: IDanhGiaColumn): number => {
    if (item.DanhSachTieuChiCon && item.DanhSachTieuChiCon.length > 0) {
      const childrenSum = item.DanhSachTieuChiCon.reduce((sum, child) => sum + updateScoreDG(child), 0);

      if ((!item.DiemThuong && item.DiemTru) || (!item.DiemTru && item.DiemThuong)) {
        // Nếu phần tử có DiemCong hoặc DiemTru, giữ nguyên DiemTuCham hiện tại
        return item.DiemDanhGia || 0;
      } else {
        // Nếu không, cập nhật DiemDanhGia bằng tổng điểm của các phần tử con
  ...
  data.forEach(updateScoreTuDG);
  data.forEach(updateScoreDG);
  return data;
};


export const useMauPhieuTTDVChamDiemColumn = ({
  dataRoot,
  scorePoint,
  dataSource,
  setDataSource,
  ...
  updatePoint: (totalPoint: number, totalPointTuDG: number, normalPointTuDG: number, bonusPointTuDG: number, penaltyPointTuDG: number, hasDiemLietTuDG: boolean,
    totalPointDG: number, normalPointDG: number, bonusPointDG: number, penaltyPointDG: number, hasDiemLietDG: boolean) => void,
}) => {
  ...
  const colNameChange = useRef<string>()
        ...
        const updateDataSourse = (items: IDanhGiaColumn[]): IDanhGiaColumn[] => {
          return items.map(item => {
            if (item.MaTieuChi === record.MaTieuChi) {
              ...
              if (colName === 'isGiaiTrinh' || colName === 'isGiaiTrinhDG') {
    ...
    if (danhGiaContext.saoChepDiemHandler) {
      ...
      function AutoFillHandler(data: IDanhGiaColumn[]) {
        data.forEach(item => {
          ...
          if (item.ThangDiem && item.ThangDiem > 0 && !item.DiemThuong && !item.DiemTru && !item.DiemDanhGia) {
            item.DiemDanhGia = item.DiemDanhGia ?? Number(item.ThangDiem)
          }
  ...
  const updateBonusAndPenaltyPoints = useCallback((data: IDanhGiaColumn[]) => {
    let normalPointsTuDG = 0;
    let bonusPointsTuDG = 0;
    let penaltyPointsTuDG = 0;
    let hasDiemLietTuDG = false

    let normalPointsDG = 0;
    let bonusPointsDG = 0;
    let penaltyPointsDG = 0;
    let hasDiemLietDG = false
      ...
      items.forEach(item => {
        ...
        if (item.DanhSachTieuChiCon && item.DanhSachTieuChiCon.length > 0) {
          if ((item.DiemThuong || item.DiemTru) && item.ThangDiem && item.DiemTuCham && (Math.abs(item.ThangDiem) == Math.abs(item.DiemTuCham))) {
            if (item.DiemThuong && !item.DiemTru)
              bonusPointsTuDG += item.DiemTuCham || 0
            if (!item.DiemThuong && item.DiemTru)
              penaltyPointsTuDG += item.DiemTuCham || 0
          }
        } else {
          if (item.DiemThuong && !item.DiemTru) {
            bonusPointsTuDG += item.DiemTuCham || 0
          ...
          if (!item.DiemThuong && !item.DiemTru) {
            normalPointsTuDG += item.DiemTuCham || 0
          }
          if (item.DiemLiet && item.isChecked) {
            hasDiemLietTuDG = true
          }
        ...
        if (item.DanhSachTieuChiCon && item.DanhSachTieuChiCon.length > 0) {
          if ((item.DiemThuong || item.DiemTru) && item.ThangDiem && item.DiemDanhGia && (Math.abs(item.ThangDiem) == Math.abs(item.DiemDanhGia))) {
            if (item.DiemThuong && !item.DiemTru)
              bonusPointsDG += item.DiemDanhGia || 0
            if (!item.DiemThuong && item.DiemTru)
              penaltyPointsDG += item.DiemDanhGia || 0
          }
        } else {
          if (item.DiemThuong && !item.DiemTru) {
            bonusPointsDG += item.DiemDanhGia || 0
          ...
          if (!item.DiemThuong && !item.DiemTru) {
            normalPointsDG += item.DiemDanhGia || 0
          }
    ...
    updatePoint(
      colNameChange.current == 'DiemTuCham'
        ? Number(normalPointsTuDG + bonusPointsTuDG + penaltyPointsTuDG)
        : colNameChange.current == 'DiemDanhGia'
          ? Number(normalPointsDG + bonusPointsDG + penaltyPointsDG)
          : Number(normalPointsDG + bonusPointsDG + penaltyPointsDG),
      Number(normalPointsTuDG + bonusPointsTuDG + penaltyPointsTuDG),
      normalPointsTuDG,
      bonusPointsTuDG,
            ...
            if (item.MaTieuChi === record.MaTieuChi) {
              ...
              if (colName === 'isGiaiTrinh' || colName === 'isGiaiTrinhDG') {
                ...
                if (updatedItem.DuocChamNhieuLan || updatedItem.DiemLiet || updatedItem.DiemThuong || updatedItem.DiemTru) {
                  updatedItem.isGiaiTrinhDG = (updatedItem.DiemTuCham != 0) || (updatedItem.DiemDanhGia != 0)
                } else updatedItem.isGiaiTrinhDG = (updatedItem.DiemTuCham != updatedItem.ThangDiem) || (updatedItem.DiemDanhGia != updatedItem.ThangDiem)
              }
              return updatedItem
            }
            if (item.DanhSachTieuChiCon) {
              ...
              if ((updatedItem.DiemThuong || updatedItem.DiemTru) && updatedChildren.length > 0) {
                ...
                const totalChildTuDG = updatedChildren.reduce((sum, child) => sum + (child.DiemTuCham || 0), 0)
                updatedItem.DiemTuCham = currentSelectValue ? currentSelectValue : Number(totalChildTuDG)

                const totalChildDG = updatedChildren.reduce((sum, child) => sum + (child.DiemDanhGia || 0), 0)
                updatedItem.DiemDanhGia = currentSelectValue ? currentSelectValue : Number(totalChildDG)
              }
  ...
  const columns: TableColumnsType<IDanhGiaColumn> = useMemo(
    () => {
      return [
        ...
        {
          title: <p style={{ textAlign: 'center' }}>Điểm Thủ trưởng cơ quan, đơn vị chấm</p>,
