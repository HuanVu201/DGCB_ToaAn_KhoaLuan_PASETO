...
import HeaederDanhGiaDetailModal from "./HeaderDanhGiaDetailModal";
...
import { CURRENTTIME_ISOSTRING, FORMAT_DATE } from "@/data";
...
import { useDanhGiaContext } from "../../contexts";
import PhuLucComponent from "./PhuLucComponent";
import { useButtonActionContext } from "@/features/DanhGiaCanBo/components/common/contexts/useButtonActionContext";

export interface IPhuLucItem {
    key: string;
    label: string;
    ...
    penaltyPoint: number,
    scorePoint: string,
    totalKhieuNai: number
}

const DanhGiaDetailModalVisible = () => {
    ...
    useEffect(() => {
        (async () => {
            if (!currNode && buttonActionContext.danhGiaModalVisible) {
                try {
                    const resBuocXuLy = await buocXuLyApi.GetBuocHienTai({ loaiBuoc: "START" })
    ...
    useEffect(() => {
        (async () => {
            if (!danhGiaContext.userDanhGia && buttonActionContext.danhGiaModalVisible && currNode) {
    ...
    }, [danhGiaContext.userDanhGia, buttonActionContext.danhGiaModalVisible, currNode])

    useEffect(() => {
        (async () => {
            if (danhGiaContext.userDanhGia && !danhGiaContext.nextNode && buttonActionContext.danhGiaModalVisible && currNode) {
    ...
    const handlerCancel = () => {
        ...
        buttonActionContext.setDanhGiaModalVisible(false)
    ...
    return <AntdModal className="chamDiemModal" okText="Tải phiếu" visible={buttonActionContext.danhGiaModalVisible} title={"Thêm mới chấm điểm"} fullsizeScrollable handlerCancel={handlerCancel}
        footer={[
            <>
    ...
    >
        ...
        >
            <HeaederDanhGiaDetailModal form={form} diemDanhGiaTotal={diemDanhGiaTotal.current} currNode={currNode} />
...
const DanhGiaDetailModal = () => (
    <DanhGiaDetailModalVisible />
);


export default DanhGiaDetailModal