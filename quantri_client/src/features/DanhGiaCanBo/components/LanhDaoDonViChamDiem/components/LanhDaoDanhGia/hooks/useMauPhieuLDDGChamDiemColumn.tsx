...
export const useMauPhieuLanhDaoDanhGiaChamDiemColumn = ({
  dataRoot,
  scorePoint,
  dataSource,
  setDataSource,
  ...
  useEffect(() => {
    if (danhGiaContext.saoChepDiemHandler) {
      ...
      function AutoFillHandler(data: IDanhGiaColumn[]) {
        data.forEach(item => {
          const diemAutoFill = danhGiaContext.thongTinPhieuChamDiem && danhGiaContext.thongTinPhieuChamDiem.diemThamMuu ? item.DiemThamMuu
            : danhGiaContext.thongTinPhieuChamDiem && danhGiaContext.thongTinPhieuChamDiem.diemNhanXet ? item.DiemNhanXet
              : item.DiemTuCham ?? 0
          ...
          if (item.DiemLiet && (item.isCheckedDG == null || item.isCheckedDG == undefined))
      ...
      const updateDataSourse = (items: IDanhGiaColumn[]): IDanhGiaColumn[] => {
        return items.map(item => {
          ...
          if (item.MaTieuChi === record.MaTieuChi) {
            ...
            const diemAutoFill = danhGiaContext.thongTinPhieuChamDiem && danhGiaContext.thongTinPhieuChamDiem.diemThamMuu ? updatedItem.DiemThamMuu
              : danhGiaContext.thongTinPhieuChamDiem && danhGiaContext.thongTinPhieuChamDiem.diemNhanXet ? updatedItem.DiemNhanXet
                : updatedItem.DiemTuCham

            if (colName === 'SoLuong') {
              updatedItem.DiemDanhGia = (updatedItem.ThangDiem || 0) * (value) * (record.DiemTru ? -1 : 1)
            }

            if (updatedItem.DiemLiet && (updatedItem.isCheckedDG == null || updatedItem.isCheckedDG == undefined))
              updatedItem.isCheckedDG = value || false
            ...
            if (colName === 'isGiaiTrinhDG') {
              ...
              if (updatedItem.DuocChamNhieuLan || updatedItem.DiemThuong || updatedItem.DiemTru) {
                if (((updatedItem.DiemThuong && !updatedItem.DiemTru) || (!updatedItem.DiemThuong && updatedItem.DiemTru)) &&
                  ...
                  updatedItem.isGiaiTrinhDG = value != (diemAutoFill || 0)
                } else {
