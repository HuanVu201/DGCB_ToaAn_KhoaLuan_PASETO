...
import HeaederLanhDaoThamMuuChamDiemDetailModal from "./LDTMHeaderDanhGiaDetailModal";
import PhuLucComponentLanhDaoThamMuuChamDiem from "./LDTMPhuLucComponent";
...
const LanhDaoThamMuuChamDiemDetailModalVisible = () => {
    ...
    useEffect(() => {
        (async () => {
            if (!danhGiaContext.userDanhGia && buttonActionContext.lanhDaoThamMuuChamDiemModalVisible && buttonActionContext.danhGiaId) {
    ...
    }, [danhGiaContext.userDanhGia, buttonActionContext.lanhDaoThamMuuChamDiemModalVisible])

    useEffect(() => {
        (async () => {
            if (danhGiaContext.userDanhGia && !danhGiaContext.nextNode && buttonActionContext.lanhDaoThamMuuChamDiemModalVisible && !danhGiaContext.disableDanhGia) {
    ...
    useEffect(() => { // Phó đơn vị tham mưu
        (async () => {
            if (buttonActionContext.danhGiaId && buttonActionContext.lanhDaoThamMuuChamDiemModalVisible) {
                buttonActionContext.setLoading(true)

                const resGetPhieuDanhGia = await danhGiaCanBoServiceApi.GetPhieuDanhGia({ _id: buttonActionContext.danhGiaId, daXem: 2 })
                if (resGetPhieuDanhGia.data.data) {
                    ...
                    if (data.mauPhieus) {
                        ...
                        data.mauPhieus.map((ele: IDanhGiaCanBo) => {
                            const item: IDanhGiaColumn = JSON.parse(ele.dataThamMuu || ele.dataNhanXet || ele.dataTuDanhGia as any)

                            const point = ele.chiTietDiemThamMuu ? ele.chiTietDiemThamMuu?.split('#')
                            ...
                            phuLucArr.push({
                                ...
                                key: ele.id,
                                totalPoint: Number(ele.diemThamMuu ?? ele.diemNhanXet ?? ele.diemTuDanhGia ?? 0),
                                ...
                                hasDiemLiet: ele.hasDiemLietThamMuu
                            } as any)
                        })
                    ...
                    form.setFieldsValue({
                        ...data as any,
                        phanLoaiThamMuu: data.phanLoaiDanhGia,
                        diemThamMuu: data.diemDanhGia
                    })
                } else {
                        ...
                        if (node.NoiDungGiaiTrinh || node.NoiDungGiaiTrinhNX || node.NoiDungGiaiTrinhTM || node.NoiDungGiaiTrinhDG) {
                            ...
                            if (node.NoiDungGiaiTrinhTM) {
                                newLichSu.push({
                                    ...
                                    fullName: danhGiaContext.thongTinPhieuChamDiem?.nguoiThamMuu || parseToken?.fullName,
    ...
    const handlerCancel = () => {
        ...
        buttonActionContext.setLanhDaoThamMuuChamDiemModalVisible(false)
    ...
    //     if (result)
    //         form.setFieldValue('phanLoaiThamMuu', result)
    //     else
    //         form.setFieldValue('phanLoaiThamMuu', undefined)
        ...
        return ({
            ...
            children: (
                <div style={{ display: activeTab === item.key ? 'block' : 'none' }}>
                    <PhuLucComponentLanhDaoThamMuuChamDiem
    ...
    return <AntdModal className="chamDiemModal" visible={buttonActionContext.lanhDaoThamMuuChamDiemModalVisible} title={"Tham mưu chấm điểm"} fullsizeScrollable handlerCancel={handlerCancel}
        footer={[
            <>
    ...
    >
        ...
        >
            <HeaederLanhDaoThamMuuChamDiemDetailModal form={form} diemDanhGiaTotal={diemDanhGiaTotal.current} />
...
const LanhDaoThamMuuChamDiemDetailModal = () => (
    <LanhDaoThamMuuChamDiemDetailModalVisible />
);


export default LanhDaoThamMuuChamDiemDetailModal