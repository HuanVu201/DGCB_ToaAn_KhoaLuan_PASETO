...
import { useLanhDaoThamMuuChamDiemContext } from '../contexts/useLDTMChamDiemContext';
  ...
  const updateScore = (item: IDanhGiaColumn): number => {
    if (item.DanhSachTieuChiCon && item.DanhSachTieuChiCon.length > 0) {
      ...
      if ((!item.DiemThuong && item.DiemTru) || (!item.DiemTru && item.DiemThuong)) {
        // Nếu phần tử có DiemCong hoặc DiemTru, giữ nguyên DiemThamMuu hiện tại
        return item.DiemThamMuu || 0;
      } else {
        // Nếu không, cập nhật DiemThamMuu bằng tổng điểm của các phần tử con
        item.DiemThamMuu = childrenSum;
        return childrenSum;
      }
    }
    return item.DiemThamMuu || 0;
...
export const useMauPhieuLanhDaoThamMuuChamDiemColumn = ({
  dataRoot,
  scorePoint,
  dataSource,
  setDataSource,
        ...
        const updateDataSourse = (items: IDanhGiaColumn[]): IDanhGiaColumn[] => {
          return items.map(item => {
            if (item.MaTieuChi === record.MaTieuChi) {
              ...
              if (colName === 'isGiaiTrinhTM')
                updatedItem.isGiaiTrinhTM = value
  ...
  useEffect(() => {
    if (danhGiaContext.saoChepDiemHandler) {
      ...
      function AutoFillHandler(data: IDanhGiaColumn[]) {
        data.forEach(item => {
          const diemAutoFill = danhGiaContext.thongTinPhieuChamDiem && danhGiaContext.thongTinPhieuChamDiem.diemNhanXet ? item.DiemNhanXet : item.DiemTuCham ?? 0

          if (item.ThangDiem && item.ThangDiem > 0 && diemAutoFill != null && !item.DiemThuong && !item.DiemTru && !item.DiemThamMuu) {
            item.DiemThamMuu = Number(diemAutoFill)
            item.isGiaiTrinhTM = false
          }

          if (!item.DiemThamMuu && ((item.DiemThuong && !item.DiemTru) || (!item.DiemThuong && item.DiemTru)) && !(item.DanhSachTieuChiCon && item.DanhSachTieuChiCon.length > 0)) {
            item.DiemThamMuu = Number(diemAutoFill)
            item.isGiaiTrinhTM = false
          }


          if (item.DiemLiet && (item.isCheckedTM == null || item.isCheckedTM == undefined))
            item.isCheckedTM = false
  ...
  const updateBonusAndPenaltyPoints = useCallback((data: IDanhGiaColumn[]) => {
    ...
    const calculatePoints = (items: IDanhGiaColumn[]) => {
      items.forEach(item => {
        ...
        if (item.DanhSachTieuChiCon && item.DanhSachTieuChiCon.length > 0) {
          if ((item.DiemThuong || item.DiemTru) && item.ThangDiem && item.DiemThamMuu && (Math.abs(item.ThangDiem) == Math.abs(item.DiemThamMuu))) {
            if (item.DiemThuong && !item.DiemTru)
              bonusPoints += item.DiemThamMuu || 0
            if (!item.DiemThuong && item.DiemTru)
              penaltyPoints += item.DiemThamMuu || 0
          }
        } else {
          if (item.DiemThuong && !item.DiemTru) {
            bonusPoints += item.DiemThamMuu || 0
          ...
          if (!item.DiemThuong && !item.DiemTru) {
            normalPoints += item.DiemThamMuu || 0
          }
          if (item.DiemLiet && item.isCheckedTM) {
            hasDiemLiet = true
          }
    ...
    setDataSource((curr) => {
      const updateDataSourse = (items: IDanhGiaColumn[]): IDanhGiaColumn[] => {
        return items.map(item => {
          ...
          if (item.MaTieuChi === record.MaTieuChi) {
            let updatedItem = { ...item, [colName]: value }
            const diemPrev = danhGiaContext.thongTinPhieuChamDiem && danhGiaContext.thongTinPhieuChamDiem.diemNhanXet ? updatedItem.DiemNhanXet : updatedItem.DiemTuCham

            if (colName === 'SoLuong') {
              updatedItem.DiemThamMuu = (updatedItem.ThangDiem || 0) * (value) * (record.DiemTru ? -1 : 1)
            }


            if (updatedItem.DiemLiet && (updatedItem.isCheckedTM == null || updatedItem.isCheckedTM == undefined))
              updatedItem.isCheckedTM = value || false


            if (colName === 'isGiaiTrinhTM') {
              updatedItem.isGiaiTrinhTM = value
            } else if (!updatedItem.NoiDungGiaiTrinhTM && !updatedItem.DinhKemTM) {
              if (updatedItem.DuocChamNhieuLan || updatedItem.DiemThuong || updatedItem.DiemTru) {
                if (((updatedItem.DiemThuong && !updatedItem.DiemTru) || (!updatedItem.DiemThuong && updatedItem.DiemTru)) &&
                  ...
                  updatedItem.isGiaiTrinhTM = value != (diemPrev || 0)
                } else {
                  updatedItem.isGiaiTrinhTM = updatedItem.DiemDanhGia != diemPrev
                }
              } else if (item.DiemLiet)
                updatedItem.isGiaiTrinhTM = updatedItem.isCheckedTM ? updatedItem.isGiaiTrinhTM : false
              else updatedItem.isGiaiTrinhTM = updatedItem.DiemThamMuu != diemPrev
            }
            return updatedItem
          }
          if (item.DanhSachTieuChiCon) {
            ...
            if ((updatedItem.DiemThuong || updatedItem.DiemTru) && updatedChildren.length > 0) {
              ...
              const totalChild = updatedChildren.reduce((sum, child) => sum + (child.DiemThamMuu || 0), 0)
              updatedItem.DiemThamMuu = currentSelectValue ? currentSelectValue : Number(totalChild)
            }
          ...
          function Recursive(datas: IDanhGiaColumn[]) {
            datas.forEach(data => {
              if (diemLietItem.tieuChi1 == data.MaTieuChi) {
                if (diemLietItem.soLanTieuChi1 && diemLietItem.soLanTieuChi1 > 0 && data.DiemThamMuu && data.ThangDiem) {
                  if ((data.ThangDiem || 0) * diemLietItem.soLanTieuChi1 <= Math.abs(data.DiemThamMuu || 0)) {
                    confirmLietArr.push(diemLietItem)
                  }
                }
                if (!diemLietItem.soLanTieuChi1 && data.DiemThamMuu && data.DiemThamMuu != 0) {
                  confirmLietArr.push(diemLietItem)
                }
              }
  ...
  const handleSelectChange = (value: number, record: IDanhGiaColumn) => {
    ...
    onRowChange(value, record, 'DiemThamMuu');
  };
  ...
  const columns: TableColumnsType<IDanhGiaColumn> = useMemo(
    () => {
      ...
      baseColumns.push({
        title: <p style={{ textAlign: 'center' }}>Điểm Phó thủ trưởng đơn vị chấm</p>,
        ...
        render: (text: string, record: IDanhGiaColumn) => {
          ...
          if (record.ThangDiem != 0) {
            if (record.DuocChamNhieuLan) {
              return (
                <Input disabled value={record.DiemThamMuu} style={{ textAlign: 'center' }} />
              )
            }
            ...
            if (record.DiemThuong || record.DiemTru) {
              if (record.DanhSachTieuChiCon && record.DanhSachTieuChiCon.length > 0) {
                if (!((record.DiemThuong && record.DiemTru) || (!record.DiemThuong && !record.DiemTru)))
                  return (<>
                    <Input disabled hidden value={record.DiemThamMuu} />
                    <AntdSelect
                      ...
                      disabled={record.isDisabledTM}
                    /></>)

              } else {
                return (<>
                  <AntdSelect
                    value={record.DiemThamMuu}
                    onChange={(value) => onRowChange(value, record, 'DiemThamMuu')}
                    options={[
                      { label: '0', value: 0 },
                      {
                        label: `${record.DiemTru ? "-" : record.DiemThuong ? '+' : ''}${record.ThangDiem}`,
            ...
            if (record.DanhSachTieuChiCon && record.DanhSachTieuChiCon.length > 0)
              ...
              return (
                <>
                  <Input
                    disabled={record.isDisabledTM || (record.DanhSachTieuChiCon && record.DanhSachTieuChiCon.length > 0)}
                    value={record.DiemThamMuu} style={{ textAlign: 'center' }}
                    onChange={(e) => onRowChange(Number(e.target.value), record, 'DiemThamMuu')}
                  />
                </>
              )
            }
            ...
            if (record.DiemLiet) {
              if (record.DanhSachTieuChiCon && !(record.DanhSachTieuChiCon.length > 0)) {
                return (<Checkbox
                  ...
                  disabled={record.JsonDiemLiet && record.JsonDiemLiet.toString().trim() != '[]' ? true : false}
                  onChange={(e) => {
                    record.isCheckedTM = !record.isCheckedTM
                    onRowChange(e.target.checked ? true : false, record, 'DiemThamMuu')
                  }} />)
              } else return (<></>)
            }
          }
          return <Input disabled value={record.DiemThamMuu} style={{ textAlign: 'center' }} />
        }
      },)
        ...
        render: (_, record, idx) => {
          ...
          return <>
            {(record.ThangDiem != 0 && (record.DiemThuong || record.DiemTru) && !((record.DiemThuong && record.DiemTru) || (!record.DiemThuong && !record.DiemTru))) || (record.DanhSachTieuChiCon && record.DanhSachTieuChiCon.length <= 0)
              ?
              !record.isGiaiTrinhTM
                ?
                <div style={{ display: 'flex', width: '100%', justifyContent: 'center' }}>
                  <div className="addGiaiTrinhBtn" onClick={() => onRowChangeGiaiTrinh(true, record, 'isGiaiTrinhTM')}>
                ...
                <>
                  <Input.TextArea style={{ width: '100%', marginBottom: 3, textAlign: 'left' }} rows={2}
                    ...
                    onMouseLeave={(e: React.MouseEvent<HTMLTextAreaElement>) => {
                      onRowChangeGiaiTrinh((e.target as HTMLTextAreaElement).value, record, 'NoiDungGiaiTrinhTM')
                    }}
                    onBlur={(e) => {
                      onRowChangeGiaiTrinh(e.target.value, record, 'NoiDungGiaiTrinhTM')
                    }}
                    onKeyDown={(e: any) => {
                    ...
                    value={textValues[record.MaTieuChi] ?? record.NoiDungGiaiTrinhTM}
                  />
                  {record.DinhKemTM
                    ?
                    <>
                      <AntdButton disabled={false} style={{ marginTop: 0, padding: 0, height: "100%" }} loading={buttonActionContext.loading} type="link" title={getFileName(record.DinhKemTM || '')} onClick={() => onViewFile(record.DinhKemTM || '')}>
                        <p style={{ fontSize: '0.8rem' }}> {getFileNameWithFixedLength(record.DinhKemTM || '')}</p>

                      </AntdButton>
                      <Popconfirm
                        title='Xoá?'
                        onConfirm={() => {
                          onRowChangeGiaiTrinh(undefined, record, 'DinhKemTM')
                        }}
                        okText='Xoá'
                        cancelText='Huỷ'
                      >
                  ...
                  <Upload
                    ...
                    onChange={(info) => {
                      if (info.file.status == "done") {
                        onRowChangeGiaiTrinh(info.file.response.data, record, 'DinhKemTM')
                      }
                    }}>
