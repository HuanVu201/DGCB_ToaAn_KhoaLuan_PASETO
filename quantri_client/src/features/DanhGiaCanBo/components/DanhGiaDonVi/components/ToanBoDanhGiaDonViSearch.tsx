...
export const ToanBoDanhGiaDonViSearch = ({ searchParams, setSearchParams, searchParamOrigins, extraButtons, setLoading, isDanhGiaDonVi = false }: {
    ...
    const [officeDatas, setOfficeDatas] = useState<ICoCauToChuc[]>()

    useEffect(() => {
        (async () => {
            if (!officeDatas) {
                const resOffice = await coCauToChucService.Search({
                    pageNumber: 1, pageSize: 10000,
                    getAllChildren: true,
                    groupCode: parseToken?.officeCode,
                    type: 'don-vi'
                })

                if (resOffice.status == 200) {
                    setOfficeDatas(resOffice.data.data)
                } else {
                    toast.error("Lấy thông tin đơn vị thất bại")
                }
            }
        })()
    }, [parseToken])
    ...
    const onFinish = (values: ISearchDanhGiaCanBo) => {
        ...
        setSearchParams((curr) => ({
            ...
            maDonVi: curr.maDonVi || searchParamOrigins.maDonVi
                ...
                <Row gutter={[0, 0]}>
                    <Col sm={12} md={12} lg={12}>
                        <Form.Item
                            ...
                            wrapperCol={{ span: 12 }}
                        >
                            <Input style={{ width: "100%" }} placeholder="Nhập tên" allowClear />
                        </Form.Item>
                    ...
                    <Col sm={12} md={12} lg={12}>
                        <Form.Item
                            label="Thời gian tự chấm điểm từ ngày"
                            name="tuNgay"
                            labelCol={{ span: 12 }}
                            wrapperCol={{ span: 12 }}
                        >
                    ...
                    <Col sm={12} md={12} lg={12}>
                        <Form.Item
                            ...
                            wrapperCol={{ span: 12 }}
                        >
                            <Input style={{ width: '100%' }} placeholder="Nhập năm đánh giá" allowClear />
                        </Form.Item>
                    </Col>
                    <Col sm={12} md={12} lg={12}>
                        <Form.Item
