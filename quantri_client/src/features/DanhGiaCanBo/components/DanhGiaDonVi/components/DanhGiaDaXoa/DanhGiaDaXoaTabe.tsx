...
import { useDanhGiaDaXoaColumn } from "../../hooks/useDanhGiaDaXoaColumn";
import KhoiPhucChamDiemModal from "../../../common/components/KhoiPhucChamDiem/KhoiPhucChamDiemModal";

function DanhGiaDaXoaTable() {
            ...
            icon: <Popconfirm
                ...
                onConfirm={async () => {
                    if (!buttonActionContext.danhGiaId) {
                        toast.error('Không có ID của phiếu cần khôi phục')
                        return
                    }
                ...
                cancelText='Huỷ'
            >
                <UndoOutlined style={{ color: "orange" }} title="Khôi phục đánh giá chấm điểm" />
            </Popconfirm>,
            key: 'KhoiPhucDanhGia'
        },
    ], [buttonActionContext.danhGiaId])
    const columns = useDanhGiaDaXoaColumn({ pageNumber: searchParams.pageNumber ?? 1, pageSize: searchParams.pageSize ?? 10, tableActions })


    const extraButtons = [
        <AntdButton className="KhoiPhucChamDiemButton"
            title="Khôi phục đánh giá chấm điểm"
            onClick={async () => {
                if (buttonActionContext.selectedDanhGias.length > 0)
                    buttonActionContext.setKhoiPhucChamDiemModalVisible(true)
                else
                    toast.info("Chưa chọn đánh giá để thực hiện khôi phục")
            }}
            icon={<UndoOutlined />}
        > Khôi phục chấm điểm</AntdButton>,
    ];
    ...
    return (<div className="ToanBoDanhGiaDonViSwapper">
        ...
        >
            ...
            <KhoiPhucChamDiemModal reload={reload} setReload={setReload} />
        </Spin>
    </div>)
}

const DanhGiaDaXoaTableSwapper = () => (
...
export default DanhGiaDaXoaTableSwapper;