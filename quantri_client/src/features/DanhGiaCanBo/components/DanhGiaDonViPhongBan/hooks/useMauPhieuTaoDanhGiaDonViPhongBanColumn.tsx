...
export const useMauPhieuTaoDanhGiaDonViPhongBanColumn = ({
  dataRoot,
  scorePoint,
  dataSource,
  setDataSource,
  ...
  const columns: TableColumnsType<IDanhGiaColumn> = useMemo(
    () => {
      return [
        ...
        {
          title: <p style={{ textAlign: 'center' }}>Điểm đơn vị tự chấm</p>,
            ...
            return <>
              {(record.ThangDiem != 0 && (record.DiemThuong || record.DiemTru) && !((record.DiemThuong && record.DiemTru) || (!record.DiemThuong && !record.DiemTru))) || (record.DanhSachTieuChiCon && record.DanhSachTieuChiCon.length <= 0)
                ...
                !record.isGiaiTrinh
                  ...
                  <>
                    <Input.TextArea
                      style={{ width: '100%', marginBottom: 3, textAlign: 'left' }}
                      rows={2}
