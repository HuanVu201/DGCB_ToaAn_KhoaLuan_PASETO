...
export const useTableDanhGiaXepLoaiDonViPhongBanColumn = ({ pageNumber = 1, pageSize = 10, tableActions }: {
    ...
    const columns = useMemo((): ColumnsType<IDanhGiaCanBo> => {
        return [
            ...
            {
                title: <p style={{ textAlign: 'center' }}>Tên cơ quan, tổ chức, đơn vị</p>,
                key: "tenDonVi",
                dataIndex: "tenDonVi",
                align: 'center',
            },
            ...
            },
            {
                ...
                render: (_, record) => {
                    ...
                    if (!record.diemLanhDaoDanhGia)
                        newAction = newAction.filter(x => x.key !== 'XoaDiemDaCham')
                    if ( !record.buocTruocId || (record.buocTruocId && record.daXem == '3'))
                        newAction = newAction.filter(x => x.key !== 'TraLai')
                    if (record.trangThai != TrangThai_ChoDanhGia)
                        newAction = newAction.filter(x => x.key == 'XemChiTietDanhGia' || x.key == 'VetXuLy')
                    return (
