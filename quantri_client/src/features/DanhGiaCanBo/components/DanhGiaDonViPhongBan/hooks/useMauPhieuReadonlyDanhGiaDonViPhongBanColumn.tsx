...
export const useMauPhieuReadonlyDanhGiaDonViPhongBanColumn = ({
    scorePoint,
    dataSource,
    expandedRowKeys,
    onExpand,
    ...
    const columns: TableColumnsType<IDanhGiaColumn> = useMemo(
        () => {
            ...
            if (danhGiaContext.thongTinPhieuChamDiem?.diemThamMuu) {
                baseColumns.push({
                    ...
                    render: (text: string, record: IDanhGiaColumn) => {
                        ...
                        if (record.DiemThuong && !record.DiemTru)
                            return <span className='diemThuongRow'> {record.DiemThamMuu ?? 0}
                            </span>
                        if (!record.DiemThuong && record.DiemTru)
                            return <span className='diemTruRow'> {record.DiemThamMuu ?? 0} </span>
            ...
            if (danhGiaContext.thongTinPhieuChamDiem?.diemLanhDaoDanhGia) {
                baseColumns.push({
                    title: <div className="text-center"><b>Điểm đánh giá, xếp loại</b></div>,
                    ...
                    render: (text: string, record: IDanhGiaColumn) => {
                        ...
                        if (record.DiemThuong && !record.DiemTru)
                            return <span className='diemThuongRow'> {record.DiemDanhGia ?? 0}
                            </span>
                        if (!record.DiemThuong && record.DiemTru)
                            return <span className='diemTruRow'> {record.DiemDanhGia ?? 0} </span>
