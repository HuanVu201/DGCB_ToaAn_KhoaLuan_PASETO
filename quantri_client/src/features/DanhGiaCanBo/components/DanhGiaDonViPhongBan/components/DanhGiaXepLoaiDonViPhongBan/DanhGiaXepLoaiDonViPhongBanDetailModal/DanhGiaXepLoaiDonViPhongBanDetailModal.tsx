...
import { IDanhGia, IDanhGiaColumn, ILichSuDanhGia, ILichSuDanhGiaSwapper } from "@/features/DanhGiaCanBo/components/common/models/phieuDanhGia";
...
const DanhGiaXepLoaiDonViPhongBanDetailModalVisible = () => {
    ...
    useEffect(() => {
        (async () => {
            if (!danhGiaContext.userDanhGia && buttonActionContext.danhGiaXepLoaiDonViPhongBanModalVisible && buttonActionContext.danhGiaId) {
    ...
    }, [danhGiaContext.userDanhGia, buttonActionContext.danhGiaXepLoaiDonViPhongBanModalVisible])
    ...
    useEffect(() => { // Trưởng đơn vị đánh giá
        (async () => {
            if (buttonActionContext.danhGiaId && buttonActionContext.danhGiaXepLoaiDonViPhongBanModalVisible) {
                buttonActionContext.setLoading(true)

                const resGetPhieuDanhGia = await danhGiaDonViServiceApi.GetPhieuDanhGia({ _id: buttonActionContext.danhGiaId, daXem: 3 })
    ...
    const handlerCancel = () => {
        ...
        buttonActionContext.setDanhGiaXepLoaiDonViPhongBanModalVisible(false)
        ...
        return ({
            ...
            children: (
                <div style={{ display: activeTab === item.key ? 'block' : 'none' }}>
                    <PhuLucDanhGiaXepLoaiDonViPhongBanComponent
    ...
    return <AntdModal className="chamDiemModal" visible={buttonActionContext.danhGiaXepLoaiDonViPhongBanModalVisible} title={"Đánh giá thông tin chấm điểm"} fullsizeScrollable handlerCancel={handlerCancel}
        footer={[
            <>
    ...
    >
        ...
        >
            <HeaderDanhGiaXepLoaiDonViPhongBanDetailModal form={form} diemDanhGiaTotal={diemDanhGiaTotal.current} />
...
const DanhGiaXepLoaiDonViPhongBanDetailModal = () => (
    <DanhGiaXepLoaiDonViPhongBanDetailModalVisible />
);


export default DanhGiaXepLoaiDonViPhongBanDetailModal