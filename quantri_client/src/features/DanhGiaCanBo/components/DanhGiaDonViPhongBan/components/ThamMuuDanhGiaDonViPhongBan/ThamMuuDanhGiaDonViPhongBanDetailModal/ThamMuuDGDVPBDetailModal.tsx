...
const ThamMuuDanhGiaDonViPhongBanDetailModalVisible = () => {
    ...
    useEffect(() => {
        (async () => {
            if (!danhGiaContext.userDanhGia && buttonActionContext.thamMuuDanhGiaDonViPhongBanModalVisible && buttonActionContext.danhGiaId) {
    ...
    }, [danhGiaContext.userDanhGia, buttonActionContext.thamMuuDanhGiaDonViPhongBanModalVisible])

    useEffect(() => {
        (async () => {
            if (danhGiaContext.userDanhGia && !danhGiaContext.nextNode && buttonActionContext.thamMuuDanhGiaDonViPhongBanModalVisible && !danhGiaContext.disableDanhGia) {
    ...
    useEffect(() => { // Phó đơn vị tham mưu
        (async () => {
            if (buttonActionContext.danhGiaId && buttonActionContext.thamMuuDanhGiaDonViPhongBanModalVisible) {
                buttonActionContext.setLoading(true)

                const resGetPhieuDanhGia = await danhGiaDonViServiceApi.GetPhieuDanhGia({ _id: buttonActionContext.danhGiaId, daXem: 2 })
    ...
    const handlerCancel = () => {
        ...
        buttonActionContext.setThamMuuDanhGiaDonViPhongBanModalVisible(false)
        ...
        return ({
            ...
            children: (
                <div style={{ display: activeTab === item.key ? 'block' : 'none' }}>
                    <PhuLucThamMuuDanhGiaDonViPhongBanComponent
    ...
    return <AntdModal className="chamDiemModal" visible={buttonActionContext.thamMuuDanhGiaDonViPhongBanModalVisible} title={"Thông tin đánh giá cơ quan, tổ chức, đơn vị"} fullsizeScrollable handlerCancel={handlerCancel}
        footer={[
            <>
    ...
    >
        ...
        >
            <HeaderThamMuuDanhGiaDonViPhongBanDetailModal form={form} diemDanhGiaTotal={diemDanhGiaTotal.current} />
...
const ThamMuuDanhGiaDonViPhongBanDetailModal = () => (
    <ThamMuuDanhGiaDonViPhongBanDetailModalVisible />
);


export default ThamMuuDanhGiaDonViPhongBanDetailModal