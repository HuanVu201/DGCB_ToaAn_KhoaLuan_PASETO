...
import { useMauPhieuXuLyKhieuNaiDanhGiaColumn } from "../../../hooks/useMauPhieuXuLyKhieuNaiDanhGiaColumn";




function PhuLucXuLyDanhGiaKhieuNaiComponent({ dataRoot, scorePoint, updateData, updatePoint }: {
    ...
    const [allowUpdateHandler, setAllowUpdateHandler] = useState<boolean>(true)

    useEffect(() => {
        if (dataRoot.DanhSachTieuChiCon) {
            setDataSource(dataRoot.DanhSachTieuChiCon)
        }
    }, [])
    ...
    //     if (dataSource) {
    //         toast('reload')
    //         const tds: NodeListOf<HTMLTableDataCellElement> = document.querySelectorAll('td.ant-table-cell');

    //         tds.forEach((td: HTMLTableDataCellElement) => {
    //             if (td.querySelector('.noiDungKhieuNai') && !td.querySelector('.isXuLyKhieuNai')) {
    //                 const parentRow = td.closest('tr');
    //                 console.log(parentRow)
    //                 if (parentRow) {
    //                     parentRow.style.backgroundColor = '#d8ffd8';
    //                 }
    //             }
    //         });
    //     }
    ...
    useEffect(() => {
        if (dataSource && dataSource.length > 0 && allowUpdateHandler) {
            ...
            setAllowUpdateHandler(false)
        }
    }, [dataSource, allowUpdateHandler])
    ...
    const columns = useMauPhieuXuLyKhieuNaiDanhGiaColumn({
        dataRoot,
        scorePoint,
        dataSource,
        setDataSource,
        expandedRowKeys,
        onExpand, updateData, updatePoint,
    });
...
export default PhuLucXuLyDanhGiaKhieuNaiComponent;
