...
export const useXuLyKhieuNaiKienNghiColumn = ({ pageNumber = 1, pageSize = 10, }: {
    ...
    const columns = useMemo((): ColumnsType<IKhieuNaiDanhGia> => {
        return [
            ...
            },
            {
                ...
                align: 'center',
                render: (_, record) => (<>
                    {record.tenPhongBan || ''} / {record.tenDonVi || ''}
                </>)
            },
            ...
            },
            {
                ...
                key: "thoiDiemTuDanhGia",
                dataIndex: "thoiDiemTuDanhGia",
                align: 'center',
                render: (_, record) => (<>
                    {record.thoiDiemTuDanhGia ? dayjs(record.thoiDiemTuDanhGia).format(FORMAT_TIME) : ''}
                </>)
            },
            {
                ...
                render: (_, record) => {
                    return (
                        <Space direction="horizontal">
                            ...
                            {record.trangThai != 'Chờ xử lý' ? null :
                                <>
                                    <EditOutlined title="Cập nhật kết quả"
                                        onClick={() => {
                                            buttonActionContext.setXuLyKhieuNaiDanhGiaModalVisible(true)
                                            buttonActionContext.setDanhGiaId(record.maPhieu)
                                            khieuNaiContext.setKhieuNaiId(record.id)
                                        }} />
                                </>
