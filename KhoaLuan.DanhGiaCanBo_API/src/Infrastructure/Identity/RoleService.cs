...
internal class RoleService : IRoleService
{
    ...
    //    (await _roleManager.Roles.Where(x => x.IsSupperAdmin != true && x.IsAdmin != true).ToListAsync(cancellationToken))
    //        .Adapt<List<RoleDto>>();
    public async Task<List<RoleDto>> GetListAsync(CancellationToken cancellationToken)
    {
        ...
        // Ánh xạ từ entity Role sang DTO RoleDto
        var roleDtos = roles.Adapt<List<RoleDto>>();

        // Ánh xạ thêm trường "Thứ Tự" từ Role entity vào "Number" trong DTO
        foreach (var role in roleDtos)
        {
            role.Number = role.ThuTu;  // Ánh xạ trường ThoTuTu thành Number
        }
