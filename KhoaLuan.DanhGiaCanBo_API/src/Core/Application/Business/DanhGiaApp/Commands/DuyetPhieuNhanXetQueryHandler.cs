...
public class DuyetPhieuNhanXetQueryHandler : IRequestHandler<DuyetPhieuNhanXetQuery, Result>
{
    ...
    public DuyetPhieuNhanXetQueryHandler(IDapperRepository dapperRepository, IVetXuLyDanhGiaService vetXuLyDanhGiaService, ICurrentUser currentUser, IRepositoryWithEvents<DanhGia> repositoryWithEvents, IUserGroupService userGroupService, IMediator mediator)
    {
    ...
    public async Task<Result> Handle(DuyetPhieuNhanXetQuery request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            foreach (Guid danhGiaId in request.Ids)
            {
                ...
                if (itemDG == null)
                    throw new NotFoundException($"DanhGia với mã: {danhGiaId} chưa được thêm vào hệ thống");
                if (itemDG.BuocHienTaiId == null)
                    throw new Exception($"DanhGia với mã: {danhGiaId} không có thông tin bước hiện tại");
                ...
                var resUpdateDanhGia = await _mediator.Send(new UpdateDanhGiaCommand()
                {
                    Id = itemDG.Id,
                    ...
                    NguoiNhanXet = request.LoaiDiem != "NhanXet" ? null : _currentUser.GetUserFullName(),
                    ...
                    NguoiNhanXetId = request.LoaiDiem != "NhanXet" ? null : _currentUser.GetUserGroupId(),
                    ...
                    ThoiGianNhanXet = request.LoaiDiem != "NhanXet" ? null : DateTime.Now,
                ...
                if (!resUpdateDanhGia.Succeeded)
                    throw new Exception("Có lỗi trong quá trình cập nhật [Business].[DanhGias]");
