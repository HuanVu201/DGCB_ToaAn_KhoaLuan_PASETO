...
public class XoaDiemLanhDaoChamCommandHandler : ICommandHandler<XoaDiemLanhDaoChamCommand>
{
    ...
    public XoaDiemLanhDaoChamCommandHandler(IDapperRepository dapperRepository, IReadRepository<DanhGia> readRepository, IVetXuLyDanhGiaService vetXuLyDanhGiaService, ICurrentUser currentUser, IRepository<DanhGia> danhGiaRepo, IReadRepository<BuocXuLy> buocXuLyRepo)
    {
    ...
    public async Task<Result> Handle(XoaDiemLanhDaoChamCommand request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            danhGia.XoaDiemLanhDaoCham(request.LoaiDiem, danhGia.NguoiNhanXet, danhGia.NguoiThamMuu);
    ...
    {
        string query = $"SELECT ID FROM {TableNames.ChiTietDanhGias} WHERE MaPhieu = @MaPhieu";
        var lstDG = (await _dapperRepository.QueryAsync<ChiTietDanhGia2Dto>(query, new { MaPhieu = maPhieu })).ToList();
        ...
        if (lstDG.Count > 0)
        {
            foreach (var item in lstDG)
            {
                ...
                if (loaiDiem == "NhanXet")
                {
                    propUpdate.Add("DiemNhanXet = NULL");
                    propUpdate.Add("DataNhanXet = NULL");
                }
                else if (loaiDiem == "ThamMuu")
                {
                ...
                string sqlQuery = $"UPDATE {TableNames.ChiTietDanhGias} SET {string.Join(", ", propUpdate)}" + whereUpdate;
                ...
                {
                    throw new Exception("Lỗi cập nhật [Business].[ChiTietDanhGias]: " + ex.Message);
                }
            }
        }
    }
