...
public class PostTieuChiTheoDoiTuongQueryWhereBuilder
{
...
public class PostTieuChiTheoDoiTuongQueryHandler : IQueryHandler<PostTieuChiTheoDoiTuongQuery, TieuChiDanhGia>
{
    ...
    public PostTieuChiTheoDoiTuongQueryHandler(IDapperRepository dapperRepository, IRepository<TieuChiDanhGia> TieuChirepository, IRepository<MauPhieuDanhGia> mauDanhGiarepository, ICacheService cacheService)
    {
    ...
    public async Task CapNhatSTTTieuChi(TieuChiDanhGia input)
    {
        var maCuoi = input.MaDayDu.Split('.')[input.MaDayDu.Split('.').Length - 1];
        var maCheck = input.MaDayDu.Replace("." + maCuoi, "");

        // string query = $"SELECT Id,ThuTu,MaDayDu FROM {TableNames.TieuChiDanhGias}";
    ...
    public async Task<List<TieuChiTheoMaNhomDoiTuongDto>> DanhSachTieuChiTheoMaNhomDoiTuong(string maNhomDoiTuong, string maDonVi = null, string checkKiemNhiem = null, string level = null)
    {
        ...
        {
           //string queryNDT = $"SELECT Id,MaBoTieuChi,Ma,DataTieuChi,Ten,DiemDatYeuCau,DiemThuong,DiemTru,LevelBoTieuChi FROM {TableNames.MauPhieuDanhGias}";
            ...
            if (lstBoTieuChuanId.Count > 0)
            {
                ...
                // string queryPLDG = $"SELECT Id,Ten,Ma,DiemToiDa,DiemToiThieu,MaBoTieuChi FROM {TableNames.XepLoaiDanhGias}";
                ...
                // string queryTC = $"SELECT * FROM  {TableNames.TieuChiDanhGias}";
                ...
                if (lstBoTieuChuanId.Count > 0)
                {
                    ...
                    // string queryBTC = $"SELECT Id,MaBoTieuChi,TenBoTieuChi,LoaiThoiGian,ThoiGian FROM  {TableNames.BoTieuChuans}";
    ...
    public async Task<Result<TieuChiDanhGia>> Handle(PostTieuChiTheoDoiTuongQuery request, CancellationToken cancellationToken)
    {
        //var input = request.data;
        Regex.Replace(request.TenTieuChi, @"[^,.!$&()=@{}[]';]+", "");
        var input = TieuChiDanhGia.Create(request.MaTieuChi, request.MaDayDu, request.TenTieuChi, request.SuDung, request.DiemTru, request.ThuTu, request.ThuTuHienThi, request.ThangDiem, request.GhiChu, request.MaMauPhieuDanhGia,
        ...
        return Result<TieuChiDanhGia>.Success(tieuChiAdd);
    }
