...
public class GetDanhSachKhieuNaiQueryWhereBuilder
{
    public static string Build(GetDanhSachKhieuNaiDanhGiaQuery req, string? userGroupId)
    {
        ...
        {
             where += $@" AND ug.OfficeCode Like N'%' + @MaDonViTK + '%'";
        }
        if (!string.IsNullOrEmpty(req.MaPhongBanTK))
        {
             where += $@" AND ug.GroupCode Like N'%' + @MaPhongBanTK + '%'";
        }
...
public class GetDanhSachKhieuNaiDanhGiaQueryHandler : IRequestHandler<GetDanhSachKhieuNaiDanhGiaQuery, PaginationResponse<KhieuNaiDanhGiaDto>>
{
    private readonly IDapperRepository _dapperRepository;
    private readonly ICurrentUser _currentUser;
    public GetDanhSachKhieuNaiDanhGiaQueryHandler(IDapperRepository dapperRepository, ICurrentUser currentUser)
    {
        _dapperRepository = dapperRepository;
        _currentUser = currentUser;
    }

    public async Task<PaginationResponse<KhieuNaiDanhGiaDto>> Handle(GetDanhSachKhieuNaiDanhGiaQuery request, CancellationToken cancellationToken)
    {
        string userGroupId = _currentUser.GetUserGroupId() ?? string.Empty;
        string where = GetDanhSachKhieuNaiQueryWhereBuilder.Build(request, userGroupId);
        string sql = $@"SELECT hs.Id,hs.LyDo,hs.DinhKemKhieuNai, hs.TrangThai, hs.KetQua, hs.MaDonVi, hs.MaDonViCha, hs.DinhKemKetQua, hs.CreatedOn,
                        ...
                        inner join Business.DanhGias as dg on hs.MaPhieu = dg.Id
inner join [Identity].[ApplicationUserGroups] ug on ug.id = hs.CreatedBy";
        if (!string.IsNullOrEmpty(request.MaDonViTK))
        {
            sql += $@" inner join Catalog.Groups gl on ug.OfficeCode = gl.GroupCode";
        }
        if (!string.IsNullOrEmpty(request.MaPhongBanTK))
        {
