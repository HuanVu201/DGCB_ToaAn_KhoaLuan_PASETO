...
public class SearchGroupQueryWhereBuilder
{
    public static string Build(SearchGroupQuery req)
    {
        ...
        {
            where += $"AND (GroupCode = @DonViQuanLy OR OfGroupCode = @DonViQuanLy) ";
        }
...
public class SearchGroupQueryHandler : IRequestHandler<SearchGroupQuery, PaginationResponse<GroupDto>>
{
    ...
    public SearchGroupQueryHandler(IDapperRepository dapperRepository, ICacheService cacheService)
    {
    ...
    public async Task<PaginationResponse<GroupDto>> Handle(SearchGroupQuery request, CancellationToken cancellationToken)
    {
        ...
        var sql = $@"SELECT Id, OfGroupId, GroupCode, GroupName, OfGroupCode, OfGroupName, Type, Catalog, OtherCatalog, GroupOrder,DeletedOn FROM {groupTableName} {where}";
        if (request.GetAllChildren == true)
        {
            ...
            if (!string.IsNullOrEmpty(request.GroupCode))
            {
                if (!string.IsNullOrEmpty(where))
                    tmpWhere += " AND GroupCode = @GroupCode";
                else
                    tmpWhere = "WHERE GroupCode = @GroupCode";
            }

            sql = $"( " +
                $"SELECT DISTINCT ID, OfGroupId, GroupCode, GroupName, OfGroupCode, OfGroupName, Type, Catalog, OtherCatalog, GroupOrder,DeletedOn FROM {groupTableName} {tmpWhere} " +
                $"  UNION ALL " +
                $" SELECT g.ID, g.OfGroupId, g.GroupCode, g.GroupName, g.OfGroupCode, g.OfGroupName, g.Type, g.Catalog, g.OtherCatalog, g.GroupOrder,g.DeletedOn " +
        ...
        var res = await _cacheService.GetOrSetAsync(request,
            async () => await _dapperRepository.QueryAsync<GroupDto>(queryFull, request),
